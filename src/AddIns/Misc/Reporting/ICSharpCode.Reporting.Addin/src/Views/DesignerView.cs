	...
	public class DesignerView : AbstractViewContent, IHasPropertyContainer, IToolsHost, IUndoHandler, IClipboardHandler
	{
		...
		void LoadDesigner(Stream stream)
		{
			...
			var componentChangeService =
				(IComponentChangeService)this.designSurface.GetService(typeof(IComponentChangeService));
		...
		static DefaultServiceContainer CreateAndInitServiceContainer()
		{
			...
			serviceContainer.AddService(typeof(IToolboxService),
				new ICSharpCode.Reporting.Addin.Services.ToolboxService());
...
//			}
			...
			LoggingService.Debug("ReportDesignerView: ComponentChanged: " +
			                     (e.Component == null ? "<null>" : e.Component.ToString()) + ", Member=" +
			                     (e.Member == null ? "<null>" : e.Member.Name) + ", OldValue=" +
			                     (e.OldValue == null ? "<null>" : e.OldValue.ToString()) + ", NewValue=" +
			                     (e.NewValue == null ? "<null>" : e.NewValue.ToString()) + "; Loading=" + loading +
...
//			MergeFormChanges();
		...
		{
			bool loading = this.loader != null && this.loader.Loading;
			LoggingService.Debug("ReportDesigner: Component added/removed/renamed, Loading=" + loading +
			                     ", Unloading=" + this.unloading);
			if (!loading && !unloading)
			{
...
//				WorkbenchSingleton.SafeThreadAsyncCall(UpdatePropertyPad);
		...
		{
			get { return ToolboxProvider.ReportingSideBar; }
		}
...
//				StartReportExplorer ();
				...
				LoggingService.Debug("FormsDesigner loaded, setting ActiveDesignSurface to " +
				                     this.designSurface.ToString());
		...
		{
			get { return this.designSurface.GetService(typeof(IDesignerHost)) as IDesignerHost; }
		}
