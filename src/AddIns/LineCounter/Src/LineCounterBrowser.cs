...
namespace LineCounterAddin
{
	...
	public partial class LineCounterBrowser : UserControl
	{
		...
		private void ScanSolution()
		{
			...
			if (solution != null) // OpenSolution is null when no solution is opened
			{
				...
				LineCountSummary summary = new LineCountSummary("All Projects",
					m_projIconMappings["{00000000-0000-0000-0000-000000000000}"]);
				...
				foreach (IProject fiProject in projects)
				{
					...
					if (fiProject.FileName.GetFileName().IndexOf("://") != -1)
					{
					...
					else
					{
						...
						lang = fiProject.TypeGuid.ToString();
					}

					int iconIndex;
#if IMPR1
					iconIndex = projectImageListHelper.GetIndex(
						IconService.GetImageForProjectType(fiProject.Language ?? "defaultLanguageName"));
#else
		...
		/// <summary>/// Scans the project items (files, usually) of
		...
		private void AddFileListItem(LineCountInfo info)
		{
			...
			if (tsmiGroupByType.Checked)
			{
				...
				if (group == null)
				{
					group = new ListViewGroup("groupType" + info.FileType.Substring(1),
						info.FileType.Substring(1).ToUpper() + " Files");
			...
			else if (tsmiGroupByProj.Checked)
			{
				...
				if (group == null)
				{
					group = new ListViewGroup("groupProj" + info.ProjectSummary.ProjectName,
						info.ProjectSummary.ProjectName + " Files");
					lvFileList.Groups.Add(group);
				}
		...
		public void Add(string toWhat, int amount)
		{
			...
			switch (toWhat)
			{
				...
				case "codelines":
					...
					break;
				case "doccmtlines":
					m_doccmtLines += amount;
					break;
				case "normcmtlines":
					m_normcmtLines += amount;
					break;
				case "blanklines":
					m_blankLines += amount;
					break;
	...
	public class LineCountInfo
	{
		...
		public LineCountInfo(string fileName, int iconIndex, LineCountSummary projectSummary) : this(fileName,
			iconIndex)
		{
